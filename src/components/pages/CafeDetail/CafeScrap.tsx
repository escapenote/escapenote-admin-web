...
const CafeScrap: React.FC<IProps> = ({ form }) => {
  ...
  const naverMapIdData = Form.useWatch('naverMapId', form);
  const [naverMapId, setNaverMapId] = useState(() => naverMapIdData);

  useEffect(() => {
    setNaverMapId(naverMapIdData);
  }, [naverMapIdData]);
          ...
          <Form.Item name="naverMapId">
            <Box flexDirection="row" gridGap="8px">
              <Input
                ...
                value={naverMapId}
                onChange={e => setNaverMapId(e.target.value)}
              />
              <Button
                type="primary"
                loading={isFinding}
